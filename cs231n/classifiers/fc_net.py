...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in xrange(self.num_layers):
            ...
            self.params['W%d' % (i + 1)] = np.random.randn(dims[i],
                                                           dims[i + 1]) * weight_scale
        # If use batch normalization, initialize the mean beta to 0, variance gamma to 1
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_scores = affine_forward(
            layer[self.num_layers - 1], self.params[WLast], self.params[bLast])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx = {}
        dx[self.num_layers], grads[WLast], grads[
            bLast] = affine_backward(dscores, cache_scores)
        ...
        for i in reversed(xrange(1, self.num_layers)):
            ...
            if not self.use_batchnorm:
                ...
                dx[i], grads['W%d' % i], grads['b%d' % i], grads['gamma%d' % i], grads[
                    'beta%d' % i] = affine_bn_relu_backward(dx[i + 1], cache_layer[i])
